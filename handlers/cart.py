...
back_scroll_btn = InlineKeyboardButton('⬅️', callback_data='get_previous_menu_item')
forward_scroll_btn = InlineKeyboardButton('➡️', callback_data='get_next_menu_item')
navigation_markup.add(back_scroll_btn, forward_scroll_btn)

admin_buttons = []

users_dict = defaultdict(User)  # Structure: { chat_id: User }
delete_from_basket_btn = InlineKeyboardButton('🔻', callback_data='delete_one_item_from_basket')
add_to_basket_btn = InlineKeyboardButton('🔺', callback_data='add_one_item_to_basket')
# counter_of_item_btn = InlineKeyboardButton('', callback_data='show_this_type_of_items_in_basket')
pay_btn = InlineKeyboardButton('Pay order 💳', callback_data='pay')


async def return_markup_with_relevant_item_counter(chat_id: int,
                                                   menu_item: tuple) -> InlineKeyboardMarkup:
    ...
    counter_btn = InlineKeyboardButton(item_counter,
...
async def scroll_menu_item_forward_or_back(callback: types.CallbackQuery) -> None:
    menu: list = await sql_get_menu()

    index_of_item_in_view = users_dict[callback.message.chat.id].index_of_current_item_in_view
    ...
    users_dict[callback.message.chat.id].index_of_current_item_in_view = index_of_item_in_view

    tmp_markup = await return_markup_with_relevant_item_counter(callback.message.chat.id,
                                                                menu[index_of_item_in_view])
...
async def send_menu_item_to_user(message: types.Message, menu_item: tuple) -> None:
    if message.chat.id not in users_dict:
        users_dict[message.chat.id] = User(message.from_user.id, message.from_user.username)

    tmp_markup = await return_markup_with_relevant_item_counter(message.chat.id, menu_item)
    await bot.send_photo(message.from_user.id, menu_item[0], f"<b><i>{menu_item[1]}</i></b>\n"
                                                             f"<i>Description</i>: {menu_item[2]}\n"
                                                             f"<i>Price</i>: {menu_item[3]} $",
                         parse_mode="html",
                         reply_markup=tmp_markup)
...
async def delete_one_item_from_basket(callback: types.CallbackQuery) -> None:
    ...
    if menu_item in users_dict[chat_id].cart:
        ...
        tmp_markup = await return_markup_with_relevant_item_counter(chat_id, menu_item)
...
async def show_alert_with_count_of_items(callback: types.CallbackQuery) -> None:
    chat_id = callback.message.chat.id
    menu: list = await sql_get_menu()
    menu_item = menu[users_dict[chat_id].index_of_current_item_in_view]
    ...
    await callback.answer(f"You've chosen {counter_of_item} {menu_item[1]}" + additional_s,
                          show_alert=True)
...
async def pay_order(callback: types.CallbackQuery) -> None:
    ...
    for menu_item, count_of_item in users_dict[chat_id].cart.items():
        order.append(LabeledPrice(label=f"{menu_item[1]} x {count_of_item}",
...
def register_handlers_general(dp: Dispatcher) -> None:
    dp.register_callback_query_handler(scroll_menu_item_forward_or_back,
                                       Text(startswith='get_'))

    dp.register_callback_query_handler(delete_one_item_from_basket,
                                       text=delete_from_basket_btn.callback_data)
    dp.register_callback_query_handler(add_one_item_to_basket,
                                       text=add_to_basket_btn.callback_data)
    dp.register_callback_query_handler(show_alert_with_count_of_items,
