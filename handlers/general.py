...
async def reload_reply_markup(
    callback: types.CallbackQuery, new_markup: InlineKeyboardMarkup
) -> None:
    """update bot message markup"""
    await callback.message.edit_reply_markup(reply_markup=new_markup)


async def send_message_directly(
    message: types.Message, message_to_user: str, reply_markup=None
) -> None:
    """safe function to send a message to users directly"""
    try:
        await bot.send_message(
            message.from_user.id, message_to_user, reply_markup=reply_markup
        )
