...
edit_item_btn = InlineKeyboardButton(
    'Edit item ✍️', callback_data='edit_item'
)
add_item_btn = InlineKeyboardButton(
    'Add new item ✅', callback_data='add_item'
)
delete_item_btn = InlineKeyboardButton(
    'Delete item ❌', callback_data='delete_item'
)
...
async def get_new_moderator_id(message: types.Message) -> None:
    ...
    await send_menu_item_to_user(
        message, menu[0], [[edit_item_btn, delete_item_btn], add_item_btn]
    )
...
async def start_new_item_uploading(callback: types.CallbackQuery) -> None:
    ...
    ):  # check if current user is moderator, then proceed fsm
...
async def load_photo(message: types.Message, state: FSMContext) -> None:
    async with state.proxy() as data:
        data['photo'] = message.photo[
            0
        ].file_id  # we're saving not all photo, but it's ID
...
def register_handlers_admin(dp: Dispatcher) -> None:
    ...
    dp.register_callback_query_handler(
        start_new_item_uploading, text='add_item', state=None
    )
    ...
    # FSM states
    dp.register_message_handler(
        load_photo, content_types=['photo'], state=FSMAdmin.awaiting_photo
    )
    dp.register_message_handler(load_name, state=FSMAdmin.awaiting_name)
    dp.register_message_handler(
        load_description, state=FSMAdmin.awaiting_description
    )
    ...
    dp.register_message_handler(
        get_new_moderator_id, commands=['moderator'], is_chat_admin=True
    )
